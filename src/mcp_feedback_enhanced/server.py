...
此模块提供 MCP (Model Context Protocol) 的增强回馈收集功能，
支持智能环境检测，自动使用 Web UI 接口。
...
# 确保 log_level 设置为正确的大写格式
fastmcp_settings = {}

# 检查环境变量并设置正确的 log_level
    ...
    try:
        ...
        for env_var in wsl_env_vars:
            if os.getenv(env_var):
                debug_log(f"侦测到 WSL 环境变量: {env_var}")
                return True
...
def is_remote_environment() -> bool:
    ...
    for env_var in SSH_ENV_VARS:
        if os.getenv(env_var):
            debug_log(f"侦测到 SSH 环境变量: {env_var}")
            return True

    # 检查远程开发环境
    for env_var in REMOTE_ENV_VARS:
        if os.getenv(env_var):
            debug_log(f"侦测到远程开发环境: {env_var}")
            return True
    ...
    if sys.platform == "win32":
        ...
        if session_name and "RDP" in session_name:
            debug_log(f"侦测到 Windows 远程桌面: {session_name}")
            return True
...
def save_feedback_to_file(feedback_data: dict, file_path: str | None = None) -> str:
    ...
    Args:
        feedback_data: 回馈数据字典
        file_path: 保存路径，若为 None 则自动产生临时文档
    ...
    # 处理图片数据：将 bytes 转换为 base64 字符串以便 JSON 串行化
    ...
    debug_log(f"回馈数据已保存至: {file_path}")
    return file_path
                ...
                if img.get("data"):
                    try:
                        ...
                        if img_base64:
                            ...
                            debug_log(
                                f"图片 {i} Base64 已准备，长度: {len(img_base64)}"
                            )
...
async def interactive_feedback(
    ...
    timeout: Annotated[int, Field(description="等待用户回馈的超时时间（秒）")] = 86400,
) -> list:
    ...
    Args:
        ...
        timeout: Timeout in seconds for waiting user feedback (default: 86400 seconds / 1 day)
    ...
    debug_log(f"环境侦测结果 - 远程: {is_remote}, WSL: {is_wsl}")
    debug_log("使用接口: Web UI")
    ...
    try:
        ...
        if result.get("images"):
            ...
            if storage.is_file_mode():
                debug_log(
                    f"文档模式: {len(result['images'])} 张图片路径已包含在文本中"
                )
            else:
...
async def launch_web_feedback_ui(project_dir: str, summary: str, timeout: int) -> dict:
    ...
    Returns:
        dict: 收集到的回馈数据
    """
    debug_log(f"启动 Web UI 接口，超时时间: {timeout} 秒")
...
def main():
    """主要入口点，用于套件运行
    收集用户的交互回馈，支持文本和图片
    此工具使用 Web UI 接口收集用户回馈，支持智能环境检测。
    ...
    调试模式：
    - 设置环境变量 MCP_DEBUG=true 可激活详细调试输出
    - 生产环境建议关闭调试模式以避免输出干扰
