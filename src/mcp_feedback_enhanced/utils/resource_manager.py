...
class ResourceManager:
    ...
    def _memory_triggered_cleanup(self, force: bool = False):
        ...
        try:
            ...
            debug_log(
                f"内存触发清理完成: 文档={cleaned_files}, 目录={cleaned_dirs}, "
    ...
    def create_temp_file(
        ...
        try:
            ...
            debug_log(f"创建临时文档失败 [错误ID: {error_id}]: {e}")
            raise
    ...
    def register_file_handle(self, file_handle: Any) -> None:
        ...
        try:
            ...
            debug_log(f"注册文档句柄: {type(file_handle).__name__}")

        except Exception as e:
            error_id = ErrorHandler.log_error_with_context(
                e, context={"operation": "注册文档句柄"}, error_type=ErrorType.FILE_IO
            )
            debug_log(f"注册文档句柄失败 [错误ID: {error_id}]: {e}")
    ...
    def unregister_temp_file(self, file_path: str) -> bool:
        ...
        try:
            if file_path in self.temp_files:
                ...
                debug_log(f"取消临时文档追踪: {file_path}")
                return True
            return False
            ...
            error_id = ErrorHandler.log_error_with_context(
                e,
                context={"operation": "取消文档追踪", "file_path": file_path},
                error_type=ErrorType.FILE_IO,
            )
            debug_log(f"取消文档追踪失败 [错误ID: {error_id}]: {e}")
            return False
        ...
        for file_path in self.temp_files.copy():
            try:
                ...
                if file_age > max_age:
                    ...
                    files_to_remove.add(file_path)
                    cleaned_count += 1
                    debug_log(f"清理过期临时文档: {file_path}")

            except Exception as e:
                error_id = ErrorHandler.log_error_with_context(
                    e,
                    context={"operation": "清理临时文档", "file_path": file_path},
                    error_type=ErrorType.FILE_IO,
                )
                debug_log(f"清理临时文档失败 [错误ID: {error_id}]: {e}")
        ...
        for handle_ref in self.file_handles.copy():
            try:
                ...
                if hasattr(handle, "close") and not handle.closed:
                    handle.close()
                    cleaned_count += 1
                    debug_log(f"关闭文档句柄: {type(handle).__name__}")
                ...
                error_id = ErrorHandler.log_error_with_context(
                    e,
                    context={"operation": "清理文档句柄"},
                    error_type=ErrorType.FILE_IO,
                )
                debug_log(f"清理文档句柄失败 [错误ID: {error_id}]: {e}")
                handles_to_remove.add(handle_ref)
    ...
    def cleanup_all(self, force: bool = False) -> dict[str, int]:
        ...
        try:
            ...
            # 清理临时文档
            results["temp_files"] = self.cleanup_temp_files(max_age=0)  # 清理所有文档
    ...
    def configure(
        ...
        Args:
            auto_cleanup_enabled: 是否激活自动清理
            cleanup_interval: 清理间隔（秒）
            temp_file_max_age: 临时文档最大年龄（秒）
        """
