...
基于 FastAPI 的 Web 用户接口主要管理类，采用现代化架构设计。
提供完整的回馈收集、图片上传、命令运行等功能。
"""
...
class WebUIManager:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int | None = None):
        ...
        if env_host:
            ...
            self.host = host
            debug_log(f"未设置 MCP_WEB_HOST 环境变量，使用缺省主机 {self.host}")
        ...
        if env_port:
            try:
                ...
                if custom_port == 0:
                    ...
                    debug_log(
                        f"MCP_WEB_PORT 值无效 ({custom_port})，必须在 1024-65535 范围内或为 0，使用缺省端口 8765"
                    )
            except ValueError:
                debug_log(
                    f"MCP_WEB_PORT 格式错误 ({env_port})，必须为数字，使用缺省端口 8765"
                )
        else:
            debug_log(f"未设置 MCP_WEB_PORT 环境变量，使用缺省端口 {preferred_port}")
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if e.errno in {
                        10048,
                        98,
                    }:  # Windows: 10048, Linux: 98 (地址已在使用中)
                        retry_count += 1
    ...
    def stop(self):
        ...
        # 停止服务器（注意：uvicorn 的 graceful shutdown 需要额外处理）
...
我已成功为 **mcp-feedback-enhanced** 项目实现了 Markdown 语法显示功能！
...
**测试状态**: ✅ 功能正常运作"""
            ...
            debug_log(f"交互回馈: {result.get('interactive_feedback', '')}")
