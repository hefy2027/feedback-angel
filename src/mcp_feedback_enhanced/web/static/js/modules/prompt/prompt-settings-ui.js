/**
 * MCP Feedback Enhanced - 提示词设置 UI 模块
 * =========================================
 * 
 * 处理设置页签中的提示词管理接口
 */
...
(function() {
    ...
    PromptSettingsUI.prototype.init = function(containerSelector) {
        ...
        if (!this.container) {
            console.error('❌ 找不到提示词设置容器:', containerSelector);
            return false;
        }
            ...
            <div class="prompt-management-section">
                <div class="prompt-management-header">
                    ...
                    <button type="button" class="btn btn-primary prompt-add-btn" id="promptAddBtn">
                        <span data-i18n="prompts.management.addNew">添加提示词</span>
                    </button>
                </div>
            ...
            <div class="prompt-settings-item ${isAutoSubmit ? 'auto-submit-prompt' : ''}" data-prompt-id="${prompt.id}">
                <div class="prompt-settings-info">
                    ...
                    <div class="prompt-settings-meta">
                        <span data-i18n="prompts.management.created">创建于</span>: ${createdDate}
                ...
                <div class="prompt-settings-actions">
                    <button type="button" class="prompt-action-btn auto-submit-btn ${isAutoSubmit ? 'active' : ''}"
                            ...
                            title="${isAutoSubmit ? '取消自动提交' : '设置为自动提交'}"
    ...
    PromptSettingsUI.prototype.handleToggleAutoSubmit = function(promptId) {
        ...
        try {
            if (prompt.isAutoSubmit) {
                ...
                this.showSuccess(this.t('prompts.management.autoSubmitCancelled', '已取消自动提交设置'));

                // 清空设置管理器中的自动提交设置
                if (this.settingsManager) {
                    this.settingsManager.set('autoSubmitPromptId', null);
                    this.settingsManager.set('autoSubmitEnabled', false);
                    console.log('🔄 已清空自动提交设置');
                } else {
                    console.warn('⚠️ settingsManager 未设置，无法清空自动提交设置');
                }
            } else {
                // 设置为自动提交
                this.promptManager.setAutoSubmitPrompt(promptId);
                this.showSuccess(this.t('prompts.management.autoSubmitSet', '已设置为自动提交提示词：') + prompt.name);

                // 更新设置管理器中的自动提交设置
                if (this.settingsManager) {
                    console.log('🔧 设置前的 autoSubmitPromptId:', this.settingsManager.get('autoSubmitPromptId'));
                    this.settingsManager.set('autoSubmitPromptId', promptId);
                    this.settingsManager.set('autoSubmitEnabled', true);
                    console.log('✅ 已设置自动提交提示词 ID:', promptId);
                    console.log('🔧 设置后的 autoSubmitPromptId:', this.settingsManager.get('autoSubmitPromptId'));
                } else {
                    console.warn('⚠️ settingsManager 未设置，无法更新自动提交设置');
                }
            }
            ...
            if (this.settingsManager && this.settingsManager.triggerAutoSubmitStateChange) {
                ...
                console.warn('⚠️ settingsManager 或 triggerAutoSubmitStateChange 方法未设置');
            }
        } catch (error) {
            this.showError(error.message);
        }
    ...
    PromptSettingsUI.prototype.updateAutoSubmitSelect = function() {
        console.log('🔄 updateAutoSubmitSelect 开始运行');
        ...
        // 同步更新设置管理器中的自动提交提示词 ID
        if (this.settingsManager) {
            console.log('🔄 updateAutoSubmitSelect 设置前:', this.settingsManager.get('autoSubmitPromptId'));
            const currentAutoSubmitEnabled = this.settingsManager.get('autoSubmitEnabled');

            if (autoSubmitPromptId) {
                // 检查状态一致性：如果设置中的 promptId 与找到的不一致，以找到的为准
                ...
                // 不自动改变 autoSubmitEnabled 的值，完全尊重用户的设置
                // updateAutoSubmitSelect 只负责同步 promptId 和下拉列表显示
            ...
            console.log('🔄 updateAutoSubmitSelect 设置后:', {
                promptId: this.settingsManager.get('autoSubmitPromptId'),
                enabled: this.settingsManager.get('autoSubmitEnabled')
            });
        } else {
            console.warn('⚠️ updateAutoSubmitSelect: settingsManager 未设置，无法同步设置');
        }
    };
    ...
    PromptSettingsUI.prototype.handlePromptSave = function(promptData, type) {
        ...
        try {
            if (type === 'add') {
                ...
                this.showSuccess(this.t('prompts.management.addSuccess', '提示词已添加'));
            } else if (type === 'edit') {
    ...
    console.log('✅ PromptSettingsUI 模块加载完成');
