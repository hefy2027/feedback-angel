...
本文件記錄了 **Feedback Angel**（原 MCP Feedback Enhanced）的所有版本更新內容。

## [v2.6.9] - 2026-06-17 - 多會話並發支援 & UI 優化

### 🌟 版本亮點
支援多會話並發回饋，新增系統提示詞功能，優化頁面佈局消除捲動條和底部空白，修復多項互動問題。

### ✨ 新功能
- 🔀 **多會話並發回饋**：支援多個 AI Agent 同時等待回饋，左側面板顯示待回覆會話列表
  - 待回覆會話列表面板（2+ 會話時自動顯示）
  - 會話間快速切換，獨立追蹤每個會話狀態
  - 前端輪詢 + WebSocket 雙通道確保會話同步
- 📌 **系統提示詞**：支援設定全域系統提示詞，每次提交回饋時自動疊加
  - 可在設定中啟用/停用
  - 自訂提示詞內容作為前綴自動附加到回饋中
- 🔄 **無會話時保持介面**：無等待會話時不再跳轉等待頁，保持上一次介面狀態

### 🚀 改進
- 📐 **頁面佈局優化**：消除右側捲動條和底部多餘空白
  - body/container 使用 `height: 100vh` + flexbox 精確佈局
  - 統計面板浮動定位優化，減小折疊時佔用空間
  - 設定項佈局模式選擇器改為橫向排列節省空間
- 🎨 **設定面板修復**：修復 flex 壓縮導致設定項不可見的問題
- 🗑️ **移除桌面端**：移除 Tauri 桌面應用支援，專注 Web UI 方案

### 🐛 問題修復
- 修復提交回饋後文字輸入框變為禁用狀態的問題
- 修復多會話面板不顯示的問題（跨執行緒 asyncio 通知 + 前端輪詢補償）
- 修復 `.combined-content` 高度未撐滿的問題
- 重構端口管理器實現

---

## [v2.6.6] - 2026-05-19 - 超時時間延長 & 專案更名

### 🌟 版本亮點
預設回饋超時時間延長至 1 天，新增圖片文件存儲模式，專案更名為 feedback-angel。

### ✨ 新功能
- 🖼️ **圖片文件存儲模式**：透過 `MCP_IMAGE_DIR` 環境變數支援將上傳圖片儲存到本地磁碟
  - 未設置 `MCP_IMAGE_DIR` → base64 模式（原有行為）
  - 設置 `MCP_IMAGE_DIR` → 文件模式（圖片存磁碟，AI 獲得文件路徑）
  - `MCP_IMAGE_MODE` 支援 `filepath`（預設）和 `url` 兩種模式
- 📦 **專案更名**：套件名從 `mcp-feedback-enhanced` 改為 `feedback-angel`
  - 新 CLI 命令：`feedback-angel`
  - 安裝方式：`uvx feedback-angel@latest`

### 🚀 改進
- ⏱️ **延長預設超時時間**：預設回饋等待超時從 600 秒（10 分鐘）改為 86400 秒（1 天）

### 🐛 問題修復
- 修復 Starlette 1.0 相容性問題
- 修復 PyO3 Python 3.14 相容性（升級至 PyO3 0.25）
- 修復 GitHub Actions 可複用工作流問題
...
**完整專案資訊：** [GitHub - mcp-feedback-enhanced](https://github.com/hefy2027/feedback-angel)
