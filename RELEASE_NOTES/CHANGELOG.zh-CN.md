...
本文档记录了 **Feedback Angel**（原 MCP Feedback Enhanced）的所有版本更新内容。

## [v2.6.9] - 2026-06-17 - 多会话并发支持 & UI 优化

### 🌟 版本亮点
支持多会话并发反馈，新增系统提示词功能，优化页面布局消除滚动条和底部空白，修复多项交互问题。

### ✨ 新功能
- 🔀 **多会话并发反馈**：支持多个 AI Agent 同时等待反馈，左侧面板显示待回复会话列表
  - 待回复会话列表面板（2+ 会话时自动显示）
  - 会话间快速切换，独立跟踪每个会话状态
  - 前端轮询 + WebSocket 双通道确保会话同步
- 📌 **系统提示词**：支持设置全局系统提示词，每次提交反馈时自动叠加
  - 可在设置中启用/禁用
  - 自定义提示词内容作为前缀自动附加到反馈中
- 🔄 **无会话时保持界面**：无等待会话时不再跳转等待页，保持上一次界面状态

### 🚀 改进
- 📐 **页面布局优化**：消除右侧滚动条和底部多余空白
  - body/container 使用 `height: 100vh` + flexbox 精确布局
  - 统计面板浮动定位优化，减小折叠时占用空间
  - 设置项布局模式选择器改为横向排列节省空间
- 🎨 **设置面板修复**：修复 flex 压缩导致设置项不可见的问题
- 🗑️ **移除桌面端**：移除 Tauri 桌面应用支持，专注 Web UI 方案

### 🐛 问题修复
- 修复提交反馈后文本输入框变为禁用状态的问题
- 修复多会话面板不显示的问题（跨线程 asyncio 通知 + 前端轮询补偿）
- 修复 `.combined-content` 高度未撑满的问题
- 重构端口管理器实现

---

## [v2.6.6] - 2026-05-19 - 超时时间延长 & 项目更名

### 🌟 版本亮点
默认反馈超时时间延长至 1 天，添加图片文档存储模式，项目更名为 feedback-angel。

### ✨ 新功能
- 🖼️ **图片文档存储模式**：通过 `MCP_IMAGE_DIR` 环境变量支持将上传图片保存到本地磁盘
  - 未设置 `MCP_IMAGE_DIR` → base64 模式（原有行为）
  - 设置 `MCP_IMAGE_DIR` → 文档模式（图片存磁盘，AI 获得文档路径）
...
### 🚀 改进
- ⏱️ **延长默认超时时间**：默认反馈等待超时从 600 秒（10 分钟）改为 86400 秒（1 天）

### 🐛 问题修复
- 修复 Starlette 1.0 兼容性问题
- 修复 PyO3 Python 3.14 兼容性（升级至 PyO3 0.25）
- 修复 GitHub Actions 可复用工作流问题
...
### 🌟 版本亮点
大幅强化会话管理功能，添加自动执行命令、导出功能和通知系统，提供更智能的开发体验。
...
- 🔨 **设置保存机制重构**: 完全移除 localStorage 依赖，改用统一的 FastAPI 后端保存机制
  ...
  - 移除防抖机制，确保设置即时保存
  - 提升设置保存的可靠性和一致性
- ✨ **设置界面美化**: 为设置页签内的各功能区块添加对应图标
  - 提升界面视觉效果和用户体验
  - 更直观的功能识别

### 🛠️ 技术改进
- 📊 **统一存储架构**: 所有设置统一使用 JSON 文档存储，确保跨环境一致性
...
### 🌟 版本亮点
添加 SSH 远程开发环境支持，解决远程开发时无法访问 Web UI 的问题。同时改进 macOS 编译支持和桌面应用稳定性，提升开发者体验。

### ✨ 新功能
- 🌐 **SSH 远程开发支持**: 添加 `MCP_WEB_HOST` 环境变量，支持设置 Web 服务器主机绑定
  - 默认为 `127.0.0.1` 确保安全性
  - 可设置为 `0.0.0.0` 允许远程访问
  - 解决 Cursor SSH Remote 等远程开发环境的访问问题
- 🍎 **macOS 编译支持增强**: 添加 `.cargo/config.toml` 配置文档
...
## [v2.5.0] - 2025-06-15 - 桌面应用进程与性能优化

### 🌟 版本亮点
全新推出跨平台桌面应用进程，支持 Windows、macOS、Linux 三大平台。同时大幅提升系统性能与稳定性，引入防抖/节流机制优化用户体验。

### ✨ 新功能
- 🖥️ **桌面应用进程**: 基于 Tauri 框架的原生桌面应用，支持 Windows x64、macOS (Intel/Apple Silicon)、Linux x64
- 📊 **会话历史本地文档存储**: 会话记录从 localStorage 改为服务器端本地文档保存，支持跨环境数据一致性
- 🔧 **多平台构建支持**: 完整的 CI/CD 流程，支持自动化多平台桌面应用构建
- 📝 **桌面模式配置**: 添加 `MCP_DESKTOP_MODE` 环境变量，支持桌面/Web 模式切换
...
- 📊 **会话历史存储改进**: 从 localStorage 改为服务器端本地文档存储，提升跨环境一致性
...
### 🛠️ 技术改进
- 🏗️ **构建流程优化**: 添加 Makefile 桌面应用构建命令，支持 debug/release 模式
- 📦 **依赖管理**: 集成 Rust 工具链，支持跨平台编译和打包
- 🔍 **开发工具增强**: 添加环境检查、构建验证、清理工具
- 📚 **文档完善**: 添加桌面应用构建指南和工作流程说明
...
### 🌟 版本亮点
将会话管理从左侧边栏迁移到独立页签，解决浏览器兼容性问题。添加音效通知系统，支持自定义音效。

### ✨ 新功能
- 🔊 **音效通知系统**: 会话更新时播放音效提醒，支持内建音效和自定义音效上传
- 📚 **会话历史管理**: 本地保存会话记录，支持导出和清理功能
- 💾 **输入框高度记忆**: 自动保存和恢复文本输入框的高度设定
- 📋 **一键复制**: 项目路径和会话ID支持点击复制

### 🚀 改进功能
- 📋 **会话管理重构**: 从左侧边栏迁移到"会话管理"页签，解决小窗口下按钮无法点击的问题
- 🎨 **界面布局优化**: AI摘要区域自动扩展，提交按钮位置调整，移除多余描述文本
- 🌐 **多语言增强**: 添加tooltip和按钮的多语言支持
...
### 🌟 版本亮点
本版本进行了重大架构重构，**完全移除 PyQt6 GUI 依赖**，转为纯 Web UI 架构，大幅简化部署和维护。同时添加多项智能功能，包括提示词管理、自动提交、会话管理等，全面提升用户体验和工作效率。

### 🔄 重大架构变更
- 🏗️ **完全移除 PyQt6 GUI**: 彻底移除桌面应用进程依赖，简化安装和部署流程
- 🌐 **纯 Web UI 架构**: 统一使用 Web 界面，支持所有平台和环境
- 📦 **依赖大幅简化**: 移除 PyQt6、相关 GUI 库等重型依赖，安装包体积显着减小
- 🚀 **部署更简单**: 无需考虑 GUI 环境配置，适用于所有开发环境

### ✨ 全新功能
- 📝 **智能提示词管理系统**:
  - 常用提示词的 CRUD 操作（添加、编辑、删除、使用）
  - 使用频率统计和智能排序
  - 快速选择和一键应用功能
  - 支持自动提交标记和优先显示
- ⏰ **自动定时提交功能**:
...
- 🔗 **连接监控系统**:
  ...
  - 自动重连机制和错误处理
  - 详细的连接统计信息
- ⌨️ **快捷键增强**: 添加 Ctrl+I 快速聚焦输入框功能 (感谢 @penn201500)
...
- 📋 **API 文档增强**: 添加提示词管理、自动提交等新功能的 API 说明
...
- 🔧 **跨平台测试增强**: 测试功能集成 WSL 检测，提升测试覆盖率

### 🚀 改进功能
- 🎯 **环境检测优化**: 改进远程环境检测逻辑，WSL 不再被误判为远程环境
- 📊 **系统信息增强**: 系统信息工具添加 WSL 环境状态显示
...
- 🌐 **语系切换修复**: 修复图片设定区域在语言切换时文本没有正确翻译的问题
- 📝 **字体可读性改善**: 调整图片设定区域的字体大小，提升文本可读性
...
### ✨ 新功能
- ⏰ **用户超时控制**: 添加可自定义的超时设置功能，支持 30 秒至 2 小时的弹性设置
- ⏱️ **倒数计时器**: 界面顶部显示实时倒数计时器，提供可视化的时间提醒
- 🖼️ **图片大小限制**: 添加图片上传大小限制设置（无限制/1MB/3MB/5MB）
- 🔧 **Base64 兼容模式**: 添加 Base64 详细模式，提升部分 AI 模型的图片识别兼容性
- 🧹 **UV Cache 管理工具**: 添加 `cleanup_cache.py` 脚本，协助管理和清理 UV cache 空间

### 🚀 改进功能
- 📚 **文档结构优化**: 刷新文档目录结构，将图片移至 `docs/{语言}/images/` 路径
- 📖 **Cache 管理指南**: 添加详细的 UV Cache 管理指南，包含自动化清理方案
...
### ✨ 新功能
- 🎨 **水平布局模式**: GUI 与 Web UI 的合并模式添加摘要与反馈的左右布局选项
...
### 🐛 问题修复
- 🔧 **图片重复粘贴**: 解决了在 Web UI 文本输入区使用 Ctrl+V 粘贴图片时的重复问题
...
### ✨ 新功能
- 🖥️ **智能窗口定位**: 添加「总是在主屏幕中心显示窗口」设置选项
- 🌐 **多屏幕支持**: 完美解决 T 字体屏幕排列等复杂多屏幕环境的窗口定位问题
...
- 📁 **集中管理**: 重新组织文档夹结构，提升维护性
- 🖥️ **界面优化**: 现代化设计和改进的用户体验

### ✨ 新功能
- 🍎 **macOS 界面优化**: 针对 macOS 用户体验进行专项改进
- ⚙️ **功能增强**: 添加设置选项和自动关闭页面功能
- ℹ️ **关于页面**: 添加关于页面，包含版本信息、项目链接和致谢内容
...
**完整项目信息：** [GitHub - mcp-feedback-enhanced](https://github.com/hefy2027/feedback-angel)
