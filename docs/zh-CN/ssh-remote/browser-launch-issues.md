...
### ⚠️ 不要手动启动
**请勿**手动执行 `uvx mcp-feedback-enhanced test --web` 等指令，这样无法与 MCP 系统集成。
...
### Q: 为什幺在 SSH Remote 环境中无法自动打开浏览器？
...
### Q: 为什幺没有接收到 MCP 新的反馈？
A: 可能是 WebSocket 连接有问题。**解决方法**：直接重新刷新浏览器页面，这会重新创建 WebSocket 连接。
